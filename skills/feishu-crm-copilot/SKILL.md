...
在第 3 步输出预览时，必须采用以下结构：
> **[图标] CRM [业务类型：如 跟进记录/新客户/新商机] 预览 [图标]**
> ---
> 🔴 **必填字段**：
> | 字段名称 | 当前提取内容 | 状态/操作 |
> | :--- | :--- | :--- |
...
在第 4 步写入成功后，**必须且只能**采用以下格式（严禁使用“报备成功”）：
...
- **🔴 禁止揣测格式 (Zero-Guessing-Format)**：
  - AI **绝对禁止**根据“直觉”或“惯例”自行设计预览/成功卡片的布局、Emoji、标题文字。
  - 所有展示格式必须 **100% 克隆** 本章节第 1 节（预览卡片）和第 2 节（成功卡片）的结构，包括 Emoji、标题顺序、表格列数、菜单文字。
  - 若发现实际展示与本章节模板存在差异（如：使用了不同的 Emoji、不同的标题文案、不同的表格结构），视为 AI 擅自揣测，触发内部报错。
