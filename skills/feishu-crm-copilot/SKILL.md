...
AI 展示结构化预览（含质检评分），然后**必须且只能**在此步骤强行挂起：
- **🔴 死亡拦截 (Fatal Blockers)**：若 🔴 必填项缺失、映射 ID 失败（找不到 record_id），**禁止生成 [1] 确认入库 按钮**。必须报错：`❌ 致命拦截：关联记录 [XXX] 查无此项，系统已物理锁死提交入口。请先行报备。`
...
- **物理断点原则**：第 3 步预览与第 4 步提交禁止在同一次用户对话轮次内发生。
...
> ...
> ⚖️ 质检评级：92 分
> ✅ 系统底座校验 100% 通过，请主事裁决： [1]确认入库 [2]纠错重写 [3]彻底撤降
...
- **禁止记忆挪用 (Anti-Hallucination)**：严禁在当前任务失败时，寻找历史对话中的“合法实体”来顶替入库。
- **禁止格式污染**：入库内容中严禁残留 `**`、`#`、`---`、`` ` `` 等 Markdown 格式符。
- **禁止虚报成功**：入库成功的反馈必须带上直达该条记录的 Record 跳转 URL 链接。